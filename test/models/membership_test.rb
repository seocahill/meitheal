...
class MembershipTest < ActiveSupport::TestCase
  ...
  test "requires user" do
    membership = Membership.new(membership_type: :standard, starts_on: Date.current)
  ...
  test "requires starts_on" do
    membership = Membership.new(user: users(:viewer), membership_type: :standard)
    assert_not membership.valid?
    assert_includes membership.errors[:starts_on], "can't be blank"
  end

  test "membership types include standard and concession" do
    assert_includes Membership.membership_types.keys, "standard"

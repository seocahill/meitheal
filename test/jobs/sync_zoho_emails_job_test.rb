...
class SyncZohoEmailsJobTest < ActiveJob::TestCase
  ...
  test "syncs emails with blank subject without crashing" do
    blank_subject_emails = [
      {
    ...
    @stub_zoho.define_singleton_method(:emails) { |folder_id:, limit:| blank_subject_emails }
    ...
    assert_predicate email.subject, :blank?
  end
